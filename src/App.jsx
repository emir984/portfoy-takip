...
export default function App() {
  ...
  const calculatedData = useMemo(() => {
    ...
    sortedTx.forEach(tx => {
      // Veri Güvenliği: Eksik verileri atla
      if (!tx.amount || !tx.price || !tx.symbol) return;
    ...
    const activeHoldings = Object.values(holdings).map(h => {
      ...
      if (manualPrice !== undefined && manualPrice !== "") currentPrice = parseFloat(manualPrice);
      else if (lastTx) currentPrice = parseFloat(lastTx.price);
      else currentPrice = h.avgCost;
    ...
    }).filter(h => h.amount > 0.000001 || Math.abs(h.realizedPL_TRY) > 0.01);

    const currentHoldings = activeHoldings.filter(h => h.amount > 0.000001);
    
    // Grafik için temizlenmiş veri (0 değerli olanları çıkar)
    const chartData = currentHoldings
      .filter(h => h.currentValueTRY > 0)
      .map(h => ({ name: h.symbol, value: h.currentValueTRY, symbol: h.symbol }));
    ...
    return {
      ...
      chartData,
      totalValueTRY: portfolioTotalValueTRY || 0, 
      totalUnrealizedPL_TRY: portfolioTotalUnrealizedPL_TRY || 0,
      totalRealizedPL_TRY: totalRealizedProfitTRY || 0, 
      totalPL_TRY: (portfolioTotalUnrealizedPL_TRY + totalRealizedProfitTRY) || 0,
      totalInvestedMoneyTRY: totalInvestedMoneyTRY || 0, 
      totalWithdrawnMoneyTRY: totalWithdrawnMoneyTRY || 0, 
  ...
  const fetchLiveRates = async () => {
    ...
    try {
      const [usd, eur, gbp] = await Promise.all([
        ...
        fetch('https://api.frankfurter.app/latest?from=GBP&to=TRY').then(r => r.json())
      ]);

      const newRates = { USD: usd.rates.TRY, EUR: eur.rates.TRY, GBP: gbp.rates.TRY, TRY: 1 };
      await setDoc(doc(db, "settings", "rates"), newRates);
      alert("Kurlar güncellendi!");
    } catch (error) {
      alert("Kur çekme hatası. İnternetinizi kontrol edin.");
    } finally {
  ...
  const handleSaveTransaction = async (e) => {
    ...
    if (isNaN(amount) || amount <= 0 || isNaN(price) || price < 0) { 
        alert("Lütfen geçerli sayısal değerler giriniz."); return; 
    }
          ...
          {activeTab === 'dashboard' && (
            <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
                <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm min-h-[350px] flex flex-col"><h3 className="font-bold text-lg mb-4">Varlık Dağılımı</h3>{calculatedData.chartData.length > 0 ? (<div className="flex-1"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={calculatedData.chartData} dataKey="value" nameKey="symbol" cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5}>{calculatedData.chartData.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />))}</Pie><Legend /></PieChart></ResponsiveContainer></div>) : <div className="flex-1 flex items-center justify-center text-slate-400">Veri yok.</div>}</div>
