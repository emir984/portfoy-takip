...
export default function App() {
  ...
  const [savedPin, setSavedPin] = useState(() => localStorage.getItem('portfolio_pin'));
  ...
  const [rates, setRates] = useState(() => {
    ...
    } catch (e) { return { USD: 32.50, EUR: 35.20, GBP: 41.10, TRY: 1 }; }
  });
  ...
  const [newPinData, setNewPinData] = useState(""); // Şifre değiştirme inputu

  const fileInputRef = useRef(null);

  // --- Persistence ---
  useEffect(() => { localStorage.setItem('portfolio_transactions_v2', JSON.stringify(transactions)); }, [transactions]);
  useEffect(() => { localStorage.setItem('portfolio_rates', JSON.stringify(rates)); }, [rates]);
  useEffect(() => { localStorage.setItem('portfolio_prices', JSON.stringify(manualPrices)); }, [manualPrices]);
  ...
  const handleLogin = (e) => {
    ...
    if (!savedPin) {
      // İlk Kurulum
      if (pinInput.length < 4) { alert("Güvenliğiniz için en az 4 haneli bir PIN belirleyin."); return; }
      ...
      else { alert("Hatalı PIN! Lütfen tekrar deneyin."); setPinInput(""); }
    }
  };

  const handleChangePin = () => {
    if (newPinData.length < 4) { alert("Yeni PIN en az 4 hane olmalıdır."); return; }
    localStorage.setItem('portfolio_pin', newPinData);
    setSavedPin(newPinData);
    setNewPinData("");
  ...
  const handleSavePrices = () => { setManualPrices(prev => ({...prev, ...priceUpdateData})); setIsPriceModalOpen(false); };
  const handleSaveTransaction = (e) => {
    e.preventDefault();
    const amount = parseFloat(formData.amount);
    const price = parseFloat(formData.price);
    if (amount <= 0 || price < 0) { alert("Geçerli değerler giriniz."); return; }
    if (formData.type === 'sell') {
      ...
      if (modalMode === 'add' && amount > currentStock) { alert(`Yetersiz stok! Elde ${currentStock} adet var.`); return; }
    }
    const newTx = { ...formData, id: modalMode === 'edit' ? editingId : Date.now(), amount, price, symbol: formData.symbol.toUpperCase() };
    if (modalMode === 'edit') setTransactions(transactions.map(t => t.id === editingId ? newTx : t));
    else setTransactions([...transactions, newTx]);
    setIsTxModalOpen(false); resetForm();
  };
  const handleDeleteTransaction = (id) => { if (window.confirm("Silmek istediğine emin misin?")) setTransactions(transactions.filter(t => t.id !== id)); };
  ...
  const handleImport = (event) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (Array.isArray(imported)) { if(window.confirm("Veriler üzerine yazılacak?")) setTransactions(imported); }
      } catch (err) { alert("Hatalı dosya."); }
    };
    reader.readAsText(file); event.target.value = null;
  };
  const handleResetPortfolio = () => { if (window.confirm("HER ŞEY SİLİNECEK?")) { setTransactions([]); setManualPrices({}); } };
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-2xl w-full max-w-md text-center animate-in fade-in zoom-in-95 duration-300">
           <div className="flex justify-center mb-6">
              <div className="bg-blue-100 p-4 rounded-full text-blue-600"><Lock size={48} /></div>
           </div>
           <h1 className="text-2xl font-bold text-slate-800 dark:text-white mb-2">
             {savedPin ? "Hoş Geldiniz" : "Güvenlik Kurulumu"}
           </h1>
           <p className="text-slate-500 mb-6">
             {savedPin ? "Portföyünüze erişmek için lütfen PIN girin." : "Verilerinizi korumak için yeni bir PIN belirleyin."}
           </p>
           ...
           <form onSubmit={handleLogin} className="space-y-4">
              <input 
                ...
                inputMode="numeric"
                autoFocus
                placeholder={savedPin ? "PIN Giriniz" : "Yeni PIN Belirleyiniz"}
                className="w-full text-center text-2xl tracking-[0.5em] font-bold p-4 rounded-xl border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 focus:ring-4 focus:ring-blue-500 outline-none transition-all"
                value={pinInput}
                onChange={(e) => setPinInput(e.target.value)}
              />
              <button type="submit" className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-lg shadow-lg transition-transform active:scale-95">
                {savedPin ? "Giriş Yap" : "Şifreyi Kaydet ve Gir"}
              </button>
           </form>
           <p className="mt-6 text-xs text-slate-400">Bu PIN sadece bu cihazda geçerlidir.</p>
        </div>
      </div>
    );
  }
      ...
      <aside className={`fixed md:sticky top-0 h-screen w-64 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 z-40 transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
          <div className="flex items-center gap-2 font-bold text-xl text-blue-600">
            <LayoutDashboard /> <span>PortföyPro</span>
          </div>
        ...
        <div className="absolute bottom-0 w-full p-4 border-t border-slate-100 dark:border-slate-700 space-y-2">
          ...
          <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg bg-red-50 hover:bg-red-100 text-red-600 text-sm font-bold transition-colors"><LogOut size={20} /> Kilitle & Çık</button>
        </div>
      </aside>
            ...
            <>
              ...
              <div className="bg-gradient-to-br from-slate-900 to-slate-800 dark:from-slate-800 dark:to-slate-900 p-6 rounded-2xl text-white shadow-lg mb-6 relative overflow-hidden">
                 ...
                 <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div className="flex items-center gap-4">
                       ...
                       <div><p className="text-slate-400 text-xs font-bold uppercase">Toplam Yatırılan</p><p className="text-2xl font-bold">{formatNumber(calculatedData.totalInvestedMoneyTRY)} TL</p></div>
                    </div>
                    <div className="flex items-center gap-4">
                       <div className="p-3 bg-white/10 rounded-xl"><ArrowUpFromLine size={24} className="text-purple-300"/></div>
                       <div><p className="text-slate-400 text-xs font-bold uppercase">Toplam Çekilen</p><p className="text-2xl font-bold">{formatNumber(calculatedData.totalWithdrawnMoneyTRY)} TL</p></div>
                    </div>
                    <div className={`flex items-center gap-4 border-l border-white/10 pl-8 ${calculatedData.netContributionTRY <= 0 ? 'text-emerald-400' : 'text-white'}`}>
                       <div>
                          {calculatedData.netContributionTRY > 0 ? (
                              <><p className="text-slate-400 text-xs font-bold uppercase">Net Maliyet</p><p className="text-3xl font-bold">{formatNumber(calculatedData.netContributionTRY)} TL</p><p className="text-xs text-slate-400 mt-1">İçerdeki para</p></>
                          ) : (
                              <><p className="text-emerald-400/80 text-xs font-bold uppercase">Net Nakit (Cepte)</p><p className="text-3xl font-bold">+{formatNumber(Math.abs(calculatedData.netContributionTRY))} TL</p><p className="text-xs text-emerald-400/70 mt-1">Ana para çıktı, kâr cebe girdi! 🎉</p></>
                          )}
                       </div>
                    </div>
                 </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                 <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                    ...
                    <div className="text-3xl font-bold text-slate-900 dark:text-white">{formatNumber(calculatedData.totalValueTRY)} TL</div>
                 </div>
                 <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border-l-4 border-l-blue-500 shadow-sm">
                    ...
                    <div className={`text-xl font-bold ${calculatedData.totalUnrealizedPL_TRY >= 0 ? 'text-blue-600' : 'text-red-500'}`}>{calculatedData.totalUnrealizedPL_TRY >= 0 ? '+' : ''}{formatNumber(calculatedData.totalUnrealizedPL_TRY)} TL</div>
                 </div>
                 <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border-l-4 border-l-emerald-500 shadow-sm">
                    ...
                    <div className={`text-xl font-bold ${calculatedData.totalRealizedPL_TRY >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>{calculatedData.totalRealizedPL_TRY >= 0 ? '+' : ''}{formatNumber(calculatedData.totalRealizedPL_TRY)} TL</div>
                 </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
                <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm min-h-[350px] flex flex-col">
                  ...
                  {calculatedData.holdings.length > 0 ? (
                    <div className="flex-1"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={calculatedData.holdings} dataKey="currentValueTRY" nameKey="symbol" cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5}>{calculatedData.holdings.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />))}</Pie><Legend /></PieChart></ResponsiveContainer></div>
                  ) : <div className="flex-1 flex items-center justify-center text-slate-400">Veri yok.</div>}
                </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[999] p-4">
           <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
             ...
             <div className="p-6 space-y-6">
                <div><h3 className="text-sm font-bold mb-2 flex gap-2"><DollarSign size={16}/> Kurlar</h3><div className="grid grid-cols-3 gap-2">{['USD', 'EUR', 'GBP'].map(c => (<div key={c}><label className="text-xs text-slate-500">{c}</label><input type="number" step="0.01" className="w-full p-1 border rounded text-sm dark:bg-slate-700" value={rates[c]} onChange={e => setRates({...rates, [c]: parseFloat(e.target.value)||0})} /></div>))}</div></div>
                <hr className="dark:border-slate-700"/>
                <div>
                   <h3 className="text-sm font-bold mb-2 flex gap-2"><KeyRound size={16}/> Şifre Değiştir</h3>
                   <div className="flex gap-2">
                      <input type="password" placeholder="Yeni PIN (Min 4 hane)" className="flex-1 p-2 border rounded text-sm dark:bg-slate-700" value={newPinData} onChange={e => setNewPinData(e.target.value)} />
                      <button onClick={handleChangePin} className="px-4 py-2 bg-slate-800 text-white rounded text-sm">Güncelle</button>
                   </div>
                </div>
                <hr className="dark:border-slate-700"/>
                <div><h3 className="text-sm font-bold mb-2 flex gap-2"><Save size={16}/> Yedek</h3><div className="flex gap-2"><button onClick={handleExport} className="flex-1 py-2 border rounded text-sm hover:bg-slate-50 dark:hover:bg-slate-700">İndir</button><button onClick={() => fileInputRef.current?.click()} className="flex-1 py-2 bg-blue-50 text-blue-600 rounded text-sm border border-blue-200">Yükle</button><input type="file" ref={fileInputRef} className="hidden" accept=".json" onChange={handleImport} /></div></div>
                <hr className="dark:border-slate-700"/>
